...
//    print("d: \(d)")
  ...
  func post(_ params: [String : AnyObject]?, url: String, altDomain: String?, callback: @escaping APICallback) {
    if !validateEvent(params: params, url: url, altDomain: altDomain, callback: callback) { return }
    
    let domain = altDomain == nil ? "" : altDomain!
    let dummyParams = ["none": "" as AnyObject] as jsonObject
    let params = params == nil ? dummyParams  : params!
  ...
  func testEventServiceSendsAppInit() {
    ...
    e.postAppInitMetric { (res, err) in
      guard let response = res as? jsonObject,
        let attribs = response["attributes"] as? jsonObject else { okExpectation.fulfill(); return }
